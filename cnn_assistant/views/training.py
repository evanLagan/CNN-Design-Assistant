...
def train_model(request):
    try:
        # Print the enitre request data
        print("Request Object: ", request.data)
        
        # Step 1: Parse the request data
        data = request.data

        # Load the dataset using the dataset ID from the request
        dataset_id = data.get('dataset_id')
        if not dataset_id:
            return JsonResponse({'error': 'Dataset ID is required'}, status=400)
        
        train_dataset, test_dataset = load_dataset(dataset_id)
        print(f"In Train Model Function: train_dataset = {train_dataset}, test_dataset = {test_dataset}")
        ...
        model.fit(train_dataset, epochs=epochs, validation_data=test_dataset, batch_size=32)

        # Step 6: Evaluate the model (Should we have another test dataset for this?)
        val_loss, val_accuracy = model.evaluate(test_dataset, verbose=0)
        ...
        print("Error in train model: ", str(e))
